...
void draw_polygons( struct matrix *polygons, screen s, color c ) {
 ...
 for (int i=0; i < polygons->lastcol-1; i+=3){
   double *N = calculate_normal(polygons, i);
   double V[] = {0.0, 0.0, 1.0};
   double product = dot_product(N, V);
   if(product > 0){
...
void add_box( struct matrix * polygons,
  ...
  add_polygon(polygons, x1, y1, z0, x0, y1, z1, x0, y1, z0);
}
    ...
    for ( longt = longStart; longt <= longStop; longt++ ) {
      ...
      if(longt != steps-1){
        add_polygon(polygons, 
                    ...
                    points->m[0][pn1], points->m[1][pn1], points->m[2][pn1]);
      }
      if(longt != 0){
        add_polygon(polygons,
                    points->m[0][p], points->m[1][p], points->m[2][p],
                    points->m[0][pn1], points->m[1][pn1], points->m[2][pn1],
                    points->m[0][pn], points->m[1][pn], points->m[2][pn]);
      }
    }
  }
  free_matrix(points);
...
void add_torus( struct matrix * polygons, 
  ...
  for ( lat = latStart; lat < latStop; lat++ ) {
    for ( longt = longStart; longt < longStop; longt++ ) {
      ...
      index = lat * steps + longt;
      int p = index; 
      int p1 = (p+1) % (steps*steps);
      int pn1 = (p + steps + 1) % (steps*steps); 
      int pn = (p + steps) % (steps*steps); 
      add_polygon(polygons, 
                  points->m[0][p], points->m[1][p], points->m[2][p],
                  points->m[0][p1], points->m[1][p1], points->m[2][p1],
                  points->m[0][pn1], points->m[1][pn1], points->m[2][pn1]);
